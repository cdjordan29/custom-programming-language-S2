...
#reader(lib "htdp-advanced-reader.ss" "lang")((modname expr-test-S2) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #t #t none #f () #f)))
;;; expr-test-S2.rkt
...
;>>>S3 and S4 Test
; see http://www.radford.edu/~itec380/2017fall/Homeworks/S4.html
; Part 1
;
; a
(check-expect(eval (parse! "let y 3 in let x 5 in ?ad x y"))
             (+ 3 5))
(check-expect(subst "y" 3 (parse! "let x 5 in ?ad x y"))
             (parse! "let x 5 in ?ad x 3"))

; b
(check-expect(eval (parse! "let y 3 in let x y in ?ad x y"))
             (+ 3 3))
(check-expect(subst "y" 3 (parse! "let x y in ?ad x y"))
             (parse! "let x 3 in ?ad x 3"))
(check-expect(subst "x" 3 (parse! "let x y in ?ad x y"))
             (parse! "let x y in ?ad 3 y"))

; c
(check-expect(eval (parse! "let x 5 in let y 3 in ?ad let x y in ?ad x y x"))
             (+ (+ 3 5) 5))
(check-expect(subst "x" 5 (parse! "let y 3 in ?ad let x y in ?ad x y x"))
             (parse! "let y 3 in ?ad let x y in ?ad 5 y 5"))
(check-expect(subst "y" 3 (parse! "let x 5 in ?ad let x y in ?ad x y x"))
             (parse! "let x 5 in ?ad let x 3 in ?ad x 3 x"))

; d
(check-expect(eval (parse! "let x 5 in |let x ?ad x 1 in ?ad x 2|"))
             (+ 6 1))
(check-expect(subst "x" 5 (parse! "|let x ?ad x 1 in ?ad x 2|"))
             (parse! "|let x ?ad 5 1 in ?ad 5 2|"))

; e
(check-expect(eval (parse! "let y let z 4 in |let y 99 in z| in |let z 5 in ?ad |let z 10 in y| ?ad y z|"))
             (+ 4 (+ 4 5)))
(check-expect(subst "y" 5 (parse! "let y let z 4 in |let y 99 in z| in |let z 5 in ?ad |let z 10 in y| ?ad y z|"))
             (parse! "let y let z 4 in |let y 99 in z| in |let z 5 in ?ad |let z 10 in 5| ?ad 5 z|"))
